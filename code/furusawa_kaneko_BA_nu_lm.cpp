...
double X_func(double a, double b){
    return (1.0/((a-b)*(a-b)))*(2*b*b*log(a/b)/(a-b)+a-3*b);
}

double dX_func_a(double a, double b){
    return (1.0/((a-b)*(a-b)*(a-b)))*(-6*b*b*log(a/b)/(a-b) + 2*b*b/a - a + 5*b);
}

double dX_func_b(double a, double b){
    return (1.0/((a-b)*(a-b)*(a-b)))*(2*b*(2+3*b/(a-b))*log(a/b) - a - 5*b);
}
...
int main(int argc, char **argv){
    ...
    double g_ratio = g_in/100.0;
    double alpha1 = alpha1_in/10000.0;
    double alpha2 = alpha2_in/10000.0;
    double alpha_unnt = alpha1 + alpha2;
    double alpha3 = 1 - alpha_unnt;
    double alpha3_g = alpha3*g_ratio;

    double mu=1e-6, m_cat=1e-3, m_in=1e-3;
    mu = 0.001;
    m_cat = 1;
    m_in = (alpha1+alpha2)/(alpha1*X_func(mu,1)+alpha2*X_func(mu+0.01,1));

    fstream f1;
    stringstream name1;
    name1 << "result/" << g_in << "_" << alpha1_in << "_" 
    << alpha2_in << "_" << nu_in << "_mean_barabasi_Ddep_LM.txt";
