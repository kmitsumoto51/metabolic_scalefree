...
const int t_mem = epsilon_inv/10;
...
int main(int argc, char **argv){
	if(argc > 1){
        alpha1_in = atoi(argv[1]);
        alpha2_in = atoi(argv[2]);
		seed = atoi(argv[3]);
	}else{
		return 1;
	}
    ...
    while(current_nodes<N){
        ...
        for(int i=0; i<current_nodes; i++){
            pa_list.push_back(i);
            pa_weight_sum += (outdeg[i] + attra);
        }

        double prob_sum = 0.0;
        for(int i=0; i<(int)pa_list.size(); i++){
            int v = pa_list[i];
            double p = (outdeg[v] + attra) / pa_weight_sum;
        ...
        for(int nl=0; nl<c_int; nl++){
            ...
            j_in[current_nodes].push_back(current_j);
            j_out[current_j].push_back(current_nodes);
    ...
    // --- 触媒配列を「j_in と同じ順序」で用意する ---
    // cat_in[v] は indeg[v] 個ぶん確保して、未設定を -1 にする
    cat_in.assign(N, {});
    for(int v=0; v<N; ++v){
        cat_in[v].assign(indeg[v], -1);
    }

    // cat_out は outdeg[i] 個ぶん（順序は j_out[i] と同じ）でOK
    cat_out.assign(N, {});
    for(int i=0; i<N; ++i){
        cat_out[i].clear();
        cat_out[i].reserve(outdeg[i]);
    }

    // --- 触媒を各エッジ i->v に割り当て、cat_out と cat_in を整合させて埋める ---
    for(int i=0; i<N; i++){
        for(int e=0; e<outdeg[i]; e++){
            int v = j_out[i][e];       // target
            int k = uniform_int(gen);  // catalyst candidate

            // 禁止条件
            if(k == i || k == v || group_i[k] == 3){
                while(true){
                    k = uniform_int(gen);
                    if(k != i && k != v && group_i[k] != 3) break;
                }
            }

            // out側：e番目の edge(i->v) に対応する触媒
            cat_out[i].push_back(k);

            // in側：j_in[v] の中で source==i になっている位置に入れる
            int pos = -1;
            for(int t=0; t<indeg[v]; ++t){
                if(j_in[v][t] == i && cat_in[v][t] == -1){
                    pos = t;
                    break;
                }
            }
            if(pos < 0){
                std::cerr << "ERROR: cannot place catalyst for edge " << i << " -> " << v << "\n";
                std::exit(1);
            }
            cat_in[v][pos] = k;
        }
        ...
        for(int t=0; t<indeg[v]; ++t){
            if(cat_in[v][t] == -1){
                std::cerr << "ERROR: cat_in not filled at v=" << v << " t=" << t
                          << " (source j_in[v][t]=" << j_in[v][t] << ")\n";
                std::exit(1);
            }
        }
    }

    // // cout << seed << " in-out ha totonotta" << endl;
    // alpha1 = alpha1_in/10000.0;
    // alpha2 = alpha2_in/10000.0;
    // alpha3 = 1.0 - alpha1-alpha2;
    ...
    // // cout << seed << " cata_list_start" << endl;
    // int temp_j, temp_k;
    // for(int i=0; i<N; i++){
    //     for(int j=0; j<outdeg[i]; j++){
    //         temp_j = j_out[i][j];
    //         temp_k = uniform_int(gen);
    //         if(temp_k == i || temp_k == temp_j || group_i[temp_k] == 3){
    //             int flg = 1;
    //             while(flg==1){
    //                 flg = 0;
    //                 temp_k = uniform_int(gen);
    ...
    //             }
    //         }
    //         // cout << i << " " << j << " " << temp_j << " " << temp_k << endl;
    //         cat_out[i].push_back(temp_k);
    //         cat_in[temp_j].push_back(temp_k);
    //         indeg[temp_j] += 1;
    //     }
    // }
    // for(int i=0; i<N; i++){
    ...
    for(int d_in = 1; d_in<51; d_in++){
        ...
        << d_in*5 << "_" << g_in << "_" << alpha1_in << "_" << alpha2_in << "_" << seed << "_barabasi_rev.txt";
    ...
    //     stringstream name2;
    //     name2 << "result/simulation/" << N << "_" << c_int << "_" << (int)(attra_ov_c*1000+0.01) << "_" 
    //     << g_in << "_" << d_in << "_" << alpha1_in << "_" << alpha2_in << "_" << seed << "_barabasi_rev_a3gdep.txt";
    //     f2.open(name2.str().c_str(), ios::out);
    //     for(int t = 0; t<max_t_step; t++){
    //         if(t%t_mem ==0){
    //             mag = 0.0; mag1 = 0.0; mag2 = 0.0; mag3 = 0.0; mag_unnt = 0.0; mag_pn = 0.0;
    ...
    // << d_in << "_" << g_in << "_" << alpha1_in << "_" << alpha2_in << "_" << seed << "_barabasi_rev_dist_steady.txt";
